...
def validate_packet(packet: Dict[str, Any]) -> None:
    assert packet["emails"], f"{packet['packet_id']}: emails empty"
    assert packet["inventory_records"], f"{packet['packet_id']}: inventory empty"
    ids = [e["email_id"] for e in packet["emails"]]
    assert len(ids) == len(set(ids)), f"{packet['packet_id']}: duplicate email ids"
    ts = [e["timestamp"] for e in packet["emails"]]
    assert ts == sorted(ts), f"{packet['packet_id']}: emails not sorted by timestamp"
    decisive_id = packet["gold"]["decisive_email_id"]
    assert decisive_id in ids, f"{packet['packet_id']}: decisive email not present"
    target = NOISE_TIERS[packet["noise_tier"]]["emails"]
    assert len(packet["emails"]) == target, f"{packet['packet_id']}: email count mismatch"
    assert packet["packet_metadata"]["distractor_count"] == len(packet["emails"]) - sum(
        1 for e in packet["emails"] if e["is_actionable"]
    ), f"{packet['packet_id']}: distractor count mismatch"


def build_llm_prompt(packet: Dict[str, Any]) -> str:
    schema = {
        "actionable": "boolean",
        "decisive_email_id": "string or null",
        "focal_sku": "string or null",
        "affected_location": "string or null",
        "disruption_type": "one of: lead_time_delay, partial_shipment, shipment_cancellation, corrected_update, none",
        "original_eta": "YYYY-MM-DD or null",
        "revised_eta": "YYYY-MM-DD or null",
    ...
    inventory_text = []
    for r in packet["inventory_records"]:
        inventory_text.append(
            f"- SKU={r['sku']}, location={r['location']}, demand_mean={r['demand_mean']}, "
            f"demand_std={r['demand_std']}, lead_time_days={r['lead_time_days']}, "
            f"current_inventory={r['current_inventory']}, on_order_qty={r['on_order_qty']}"
        )

    prompt = f"""You are extracting actionable supplier disruption information from noisy email traffic.

Task:
Read the email packet and return exactly one JSON object. Do not include markdown fences or explanations.

Decision rule:
- Mark actionable=true only if there is a real disruption affecting an inbound order for a specific SKU/location that should change planning.
- Ignore distractors, resolved issues for other SKUs, and administrative notes.
- If there is a correction, use the latest decisive update.
- If shipment is canceled with no replacement date, revised_eta should be null and delay_days should be null.
- If the packet is not actionable, return actionable=false and null for all other fields except disruption_type="none".
...
def build_llm_request(packet: Dict[str, Any]) -> Dict[str, Any]:
    return {
...
def generate_packet(packet_id: str, split: str, pattern: str, noise_tier: str, replicate: int, rng: random.Random) -> Dict:
    ...
    packet = {
        ...
        "packet_metadata": {
            ...
            "distractor_count": len(emails) - sum(1 for e in emails if e["is_actionable"]),
...
def summarize(dataset: Dict[str, List[Dict]]) -> Dict[str, Any]:
    all_packets = dataset["pilot"] + dataset["scored"]
    by_noise = {}
    by_pattern = {}
    for p in all_packets:
        by_noise[p["noise_tier"]] = by_noise.get(p["noise_tier"], 0) + 1
        by_pattern[p["pattern"]] = by_pattern.get(p["pattern"], 0) + 1
    return {
        "seed": SEED,
        "base_date": iso(BASE_DATE),
        "pilot_count": len(dataset["pilot"]),
        "scored_count": len(dataset["scored"]),
        "total_count": len(all_packets),
        "by_noise": by_noise,
        "by_pattern": by_pattern,
        "avg_emails_per_scored_packet": round(sum(len(p["emails"]) for p in dataset["scored"]) / len(dataset["scored"]), 2),
    }
...
def main() -> None:
    ...
    outputs_dir = ensure_dir(root / "outputs")

    dataset = generate_dataset()
    write_jsonl(data_dir / "pilot_packets.jsonl", dataset["pilot"])
    write_jsonl(data_dir / "scored_packets.jsonl", dataset["scored"])

    llm_requests = [build_llm_request(p) for p in dataset["scored"]]
    write_jsonl(outputs_dir / "llm_requests.jsonl", llm_requests)

    summary = summarize(dataset)
    with (outputs_dir / "packet_summary.json").open("w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)

    print("Generated files:")
    print(f"- {data_dir / 'pilot_packets.jsonl'}")
    print(f"- {data_dir / 'scored_packets.jsonl'}")
    print(f"- {outputs_dir / 'llm_requests.jsonl'}")
    print(f"- {outputs_dir / 'packet_summary.json'}")
